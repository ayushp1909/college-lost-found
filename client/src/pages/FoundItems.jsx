  ...
  useEffect(() => {
    const fetchFoundItems = async () => {
      try {
        ...
        // Filter only items with type === "found"
        const found = (data.items || []).filter((item) => item.type === 'found');
        setItems(found);
      } catch (err) {
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <div>
          <h2>Found Items</h2>
          <p className="page-subtitle">Browse items found across campus waiting to be claimed</p>
        </div>
      ...
      {loading && <div className="loading-state">Loading found items...</div>}
